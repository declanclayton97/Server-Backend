...
app.post('/api/purchasing/remove-po-row-live', express.json(), async (req, res) => {
  if (!purchasingAuto.isLiveConfigured()) return res.status(503).json({ error: 'Live BP creds not configured' });
  const b = req.body || {};
  try { res.json(await purchasingAuto.removePoRowLive({ poId: Number(b.poId), sku: b.sku, expectQty: b.expectQty, execute: b.execute === true })); }
  catch (e) { res.status(400).json({ error: e.message }); }
});
...
app.get('/api/purchasing/sterling-portal', async (req, res) => {
  try {
    ...
    if (step === 'basket') return res.json({ ok: true, step, ...(await sp.sterlingBasket({ jar, sample: req.query.raw === '1', from: req.query.from })) });
    if (step === 'trace') return res.json({ step, ...(await sp.sterlingLoginTrace()) });
...
app.post('/api/purchasing/sterling-worker-dry', express.json({ limit: '2mb' }), async (req, res) => {
  try {
    ...
    if (req.body.ordersList) payload.opts = { ordersList: true };
...
app.post('/api/purchasing/supplier-scheduled-run', express.json(), async (req, res) => {
  ...
  try {
    const b = { ...req.query, ...(req.body || {}) };
    res.json(await purchasingSchedule.runSupplierScheduled({ pool, altItemsUrl: ALT_ITEMS_URL, supplier: b.supplier || 'FRISTADS', dryRun: b.dryRun === '1' || b.dryRun === true, force: b.force === '1' || b.force === true, forcePlace: b.forcePlace === '1' || b.forcePlace === true, excludeSkus: Array.isArray(b.excludeSkus) ? b.excludeSkus : String(b.excludeSkus || '').split(',').map((x) => x.trim()).filter(Boolean) }));
  } catch (e) { res.status(400).json({ error: e.message }); }
});
...
app.post('/api/debug/bp-setref', async (req, res) => {
  try {
    const { orderId, reference, client, op, data, fields } = req.body || {};
    if (!orderId) return res.status(400).json({ error: 'orderId required' });
    const opts = client ? { client } : {};
    const r = fields ? await bpLockedValidateOrder(orderId, fields, opts)
                     : op ? await bpOrderAjaxPost(orderId, op, data || {}, opts)
                     : await bpUpdateOrderReference(orderId, String(reference), opts);
    res.json(r);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/purchasing/prepare-supplier-order', async (req, res) => {
  ...
  try {
    ...
    const lines = [];
    for (const o of plan.orders) for (const l of o.lines) lines.push({ orderId: o.orderId, ref: o.ref, sku: l.sku, name: l.name, qty: l.qty, size: l.size, colour: l.colour, productId: l.productId, cost: l.cost });
    // "<name> <colour> <size>" for a line (skips blanks).
    const itemLabel = (l) => [l.name, l.colour, l.size].filter(Boolean).join(' ');
    ...
    // order / cancel / swap).
    const agg = {};
    for (const l of lines) { if (!agg[l.sku]) agg[l.sku] = { qty: 0, name: l.name, colour: l.colour, size: l.size }; agg[l.sku].qty += l.qty; }
    // name/colour/size carried through for suppliers whose basket resolves by product
    // attributes (Mascot=name; Prestige=name+colour+size); ignored by the SKU-keyed ones.
    const importLines = Object.entries(agg).map(([stockCode, v]) => ({ stockCode, qty: v.qty, name: v.name, colour: v.colour, size: v.size }));
    ...
    if (!dryRun) {
      if (createPo) {
        ...
        // Order" PO (child of the main) = the shortfall qty. Create both first so
        // the notes can cross-reference each other's PO#.
        ...
        const mainSrc = lines.filter((l) => l.order > 0).sort((a, b) => a.orderId - b.orderId);
        const shortSrc = outOfStock.slice().sort((a, b) => a.orderId - b.orderId);

        po = await purchasingAuto.createSupplierPO(supplier, mainItems, { reference: `Auto-PO ${plan.supplier}` });
        if (boItems.length) backorderPo = await purchasingAuto.createSupplierPO(supplier, boItems, { reference: `Auto-PO ${plan.supplier} BACK ORDER`, status: purchasingAuto.PO_BACKORDER_STATUS, parentOrderId: po && po.poId ? po.poId : undefined });

        // main PO note — items grouped by order, plus any split-to-back-order lines
        if (po && po.poId) {
          let note = 'This purchase order was created using the following sales orders:\n'
            + mainSrc.map((l) => `${l.order} of ${itemLabel(l)} from order SO#${l.orderId}`).join('\n');
          if (backorderPo && backorderPo.poId) note += '\n\n' + shortSrc.map((l) => `${itemLabel(l)} split to back order to PO#${backorderPo.poId}`).join('\n');
          try { await purchasingAuto.addOrderNote(po.poId, note, supContact); } catch (e) { console.error('[purchasing] main note', e.message); }
        }
        // back-order PO note — split from the main PO, items grouped by order
        if (backorderPo && backorderPo.poId) {
          const note = `This back order was split from PO#${po && po.poId ? po.poId : '?'}, using the following sales orders:\n`
            + shortSrc.map((l) => `${l.short} of ${itemLabel(l)} from order SO#${l.orderId}`).join('\n');
          try { await purchasingAuto.addOrderNote(backorderPo.poId, note, supContact); } catch (e) { console.error('[purchasing] bo note', e.message); }
        }
      ...
      const BASKET_SUPPLIERS = new Set(['SNICKERS', 'PORTWEST', 'BLAKLADER', 'MASCOT', 'PRESTIGE', 'BTC', 'CASTLE', 'FRISTADS', 'LEO', 'BEHRENS', 'STEEL BLUE', 'STEELBLUE', 'CHADWICK', 'CHADWICK TEXTILES', 'ROCK FALL', 'ROCKFALL', 'ENGEL', 'ULTIMATE INDUSTRIAL', 'ULTIMATE', 'PUMA', 'SAFETY JOGGER', 'SAFETYJOGGER', 'BEESWIFT', 'ALEXANDRA', 'COTTON RIDGE', 'COTTONRIDGE', 'ORBIT', 'ORBIT INTERNATIONAL']); // suppliers with a built basket flow (Beeswift needs BEESWIFT_TOKEN on Alt-Items)
      ...
      if (po && po.poId && (req.body.emailPo === true || EMAIL_SUPPLIERS.has(String(supplier).toUpperCase()))) {
        ...
        // Recipient = the order email ON the PO (supplier party snapshot) — the
        // authoritative address; then req.emailTo / the supplier's BP contact / env.
        let recipient = req.body.emailTo || (await purchasingAuto.poSupplierEmail(po.poId)) || (await purchasingAuto.supplierEmailOf(supplier)) || supplierOrderEmail(supplier);
      ...
      if (outOfStock.length && backorderPo && backorderPo.poId) {
        ...
        for (const oid of Object.keys(shortByOrder)) {
          const noteText = shortByOrder[oid].map((l) => `${l.short} of ${itemLabel(l)} split to back order PO#${backorderPo && backorderPo.poId ? backorderPo.poId : '?'}${l.stock && l.stock.deldate ? `. Next delivery ${l.stock.deldate}` : ''}`).join('\n');
