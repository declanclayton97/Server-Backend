...
app.post('/api/purchasing/prepare-supplier-order', async (req, res) => {
  ...
  try {
    ...
    if (!dryRun) {
      ...
      const BASKET_SUPPLIERS = new Set(['SNICKERS', 'PORTWEST', 'BLAKLADER', 'MASCOT', 'PRESTIGE', 'BTC', 'CASTLE', 'FRISTADS', 'LEO', 'BEHRENS', 'STEEL BLUE', 'STEELBLUE', 'CHADWICK', 'CHADWICK TEXTILES', 'ROCK FALL', 'ROCKFALL', 'ENGEL', 'ULTIMATE INDUSTRIAL', 'ULTIMATE', 'PUMA', 'SAFETY JOGGER', 'SAFETYJOGGER', 'BEESWIFT', 'ALEXANDRA', 'COTTON RIDGE', 'COTTONRIDGE']); // suppliers with a built basket flow (Beeswift needs BEESWIFT_TOKEN on Alt-Items)
